...
const EnrollStudent = ({ onLogout }) => {
  ...
  useEffect(() => {
    ...
    if (error) {
      triggerAlert(
        "error",
        "Error",
        error?.data?.error || "Student not eligiable to enroll."
      );
  ...
  const addCourse = () => {
    const newCourse =
      eligiableCourses.length > 0 ? { ...eligiableCourses[0] } : {}; // Make a copy of the first eligible course or an empty object
